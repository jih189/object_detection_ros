...
int main(int argc, char **argv)
{
  ...
  desc.add_options()("help,h", "produce help message")("obj-name,o", po::value<std::string>(&obj_name), "name of traget object")("sample-step,s", po::value<float>(&sample_step)->default_value(0.005f), "sample step")("depth,d", po::value<float>(&depth)->default_value(0.3f), "distance between object and camera")("param-linefit,l", po::value<std::string>(&str_param_linfit)->default_value(std::string("para_template_line_fitter.txt")), "set parameters for line fitting")("save-path,p", po::value<std::string>(&str_result_path)->default_value(std::string("data_newobject")), "set result path")

      ("width", po::value<int>(&width)->default_value(640), "width")("height", po::value<int>(&height)->default_value(480), "height")("intrinsic", po::value<std::string>(&intrinsic)->default_value("Intrinsics_normal.xml"), "intrinsic parameters")("dull_edge", po::value<bool>(&dull_edge)->default_value(true), "consider dull edges");
  po::variables_map vm;
  ...
  int maxd = 16;
  std::string tempname = obj_name;
  // for getting better edge, sample step need to be small for template generate.
  ...
  while (key != 27) // until 'esc'
  {
    ...
    //cObjModel.keepOnlyContourPoints();
    cObjModel.getVisibleArea(height, width, true, img_result);
    // Find visible edges
    ...
    for (i = 1; i < int(vsp.size()); i++)
    {
      if (edge_mem == vsp[i].edge_mem &&
          ((vsp[i].coord3.x - vsp[i - 1].coord3.x) * (vsp[i].coord3.x - vsp[i - 1].coord3.x) +
           (vsp[i].coord3.y - vsp[i - 1].coord3.y) * (vsp[i].coord3.y - vsp[i - 1].coord3.y) +
           (vsp[i].coord3.z - vsp[i - 1].coord3.z) * (vsp[i].coord3.z - vsp[i - 1].coord3.z)) <= sample_step * sample_step * 1.1)
      {
        // skip over
      }
      else
    ...
    case 'k':
      ...
      lf.FitLine(img);
      sprintf(buf, "/edge_template_line%03d.png", int_not);
      lf.DisplayEdgeMap(img, (str_result_path + buf).c_str());
      cvReleaseImage(&img);

      sprintf(buf, "/edge_template%03d.txt", int_not);
