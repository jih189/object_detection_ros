...
void CObjectModel::loadKeyframes(string obj_name)
{
  ...
  while (fsk.is_open())
  {
    ...
    std::stringstream ss;
    ss << data_dir << "/"
       << "keyframe" << std::setw(3) << std::setfill('0') << num_keyframes_ + 1 << ".jpg";
...
void CObjectModel::saveKeyframe(std::string obj_name, IplImage *imgG, CvMat *pose)
{
  ...
  ss.str(std::string());
  ss << data_dir << "/"
     << "pose" << std::setw(3) << std::setfill('0') << num_keyframes_ + 1 << ".xml";
  ...
  if (new2Dpt.size()) // One or more keypoints
  {
    ...
    ss.str(std::string());
    ss << data_dir << "/"
       << "keypoint2d" << std::setw(3) << std::setfill('0') << num_keyframes_ + 1 << ".xml";
    cvSave(ss.str().c_str(), pt2d);

    ss.str(std::string());
    ss << data_dir << "/"
       << "keypoint3d" << std::setw(3) << std::setfill('0') << num_keyframes_ + 1 << ".xml";
    cvSave(ss.str().c_str(), pt3d);

    ss.str(std::string());
    ss << data_dir << "/"
       << "descriptor" << std::setw(3) << std::setfill('0') << num_keyframes_ + 1 << ".xml";
...
{
  visible_sample_points_.clear();
  visible_sample_points_.resize(sharp_sample_points_.size());

  for (int i = 0; i < sharp_sample_points_.size(); i++)
  {
    visible_sample_points_[i].coord3 = sharp_sample_points_[i];
    visible_sample_points_[i].coord2 = project3Dto2D(sharp_sample_points_[i]);
    visible_sample_points_[i].edge_mem = sharp_sample_points_edge_indices_[i];
  }
}
...
#pragma omp parallel for
  for (int i = 0; i < (int)sharp_edges_.size(); i++)
  {
    ...
    if (deg[i] < -67.5)
      nidx[i] = 0;
    else if (-67.5 <= deg[i] && deg[i] < -22.5)
      nidx[i] = 1;
    else if (-22.5 <= deg[i] && deg[i] < 22.5)
      nidx[i] = 2;
    else if (22.5 <= deg[i] && deg[i] < 67.5)
      nidx[i] = 3;
    else if (67.5 <= deg[i] && deg[i] < 112.5)
      nidx[i] = 0;
    else if (112.5 <= deg[i] && deg[i] < 157.5)
      nidx[i] = 1;
    else if (157.5 <= deg[i] && deg[i] < 202.5)
      nidx[i] = 2;
    else if (202.5 <= deg[i] && deg[i] < 247.5)
      nidx[i] = 3;
    else if (247.5 <= deg[i])
      nidx[i] = 0;
  }
...
#pragma omp parallel for
  for (int i = 0; i < (int)dull_boundary_edges_.size(); i++)
  {
    ...
    if (deg[i + j] < -67.5)
      nidx[i + j] = 0;
    else if (-67.5 <= deg[i + j] && deg[i + j] < -22.5)
      nidx[i + j] = 1;
    else if (-22.5 <= deg[i + j] && deg[i + j] < 22.5)
      nidx[i + j] = 2;
    else if (22.5 <= deg[i + j] && deg[i + j] < 67.5)
      nidx[i + j] = 3;
    else if (67.5 <= deg[i + j] && deg[i + j] < 112.5)
      nidx[i + j] = 0;
    else if (112.5 <= deg[i + j] && deg[i + j] < 157.5)
      nidx[i + j] = 1;
    else if (157.5 <= deg[i + j] && deg[i + j] < 202.5)
      nidx[i + j] = 2;
    else if (202.5 <= deg[i + j] && deg[i + j] < 247.5)
      nidx[i + j] = 3;
    else if (247.5 <= deg[i + j])
      nidx[i + j] = 0;
  }
...
#pragma omp parallel for
  for (int i = 0; i < int(visible_sample_points_.size()); i++)
  {
    ...
    if (_nidx == 0)
      visible_sample_points_[i].normal_ang = M_PI / 2;
    else if (_nidx == 1)
      visible_sample_points_[i].normal_ang = -M_PI / 4;
    else if (_nidx == 2)
      visible_sample_points_[i].normal_ang = 0;
    else if (_nidx == 3)
      visible_sample_points_[i].normal_ang = M_PI / 4;
  ...
  while (iter < k && iter < double(N))
  {
    ...
    for (int i = 0; i < nop; i++)
    {
      ...
      CV_MAT_ELEM(*x2d_proj, float, 2, i) = sqrt(
          (CV_MAT_ELEM(*x2d_proj, float, 0, i) - visible_sample_points_[valid_idx[i]].edge_pt2.x) * (CV_MAT_ELEM(*x2d_proj, float, 0, i) - visible_sample_points_[valid_idx[i]].edge_pt2.x) +
          (CV_MAT_ELEM(*x2d_proj, float, 1, i) - visible_sample_points_[valid_idx[i]].edge_pt2.y) * (CV_MAT_ELEM(*x2d_proj, float, 1, i) - visible_sample_points_[valid_idx[i]].edge_pt2.y));
    }
...
void CObjectModel::loadObjectCADModel(const std::string &obj_name)
{
  ...
  if (Mobj)
  {
    ...
    for (int i = 0; i < size_vSEP; i++)
      sharp_edges_[i].first = getPoint(Mobj, i + start_idx);
    start_idx += size_vSEP;
    for (int i = 0; i < size_vSEP; i++)
      sharp_edges_[i].second = getPoint(Mobj, i + start_idx);
    start_idx += size_vSEP;
    for (int i = 0; i < size_vDEP; i++)
      dull_edges_[i].first = getPoint(Mobj, i + start_idx);
    start_idx += size_vDEP;
    for (int i = 0; i < size_vDEP; i++)
      dull_edges_[i].second = getPoint(Mobj, i + start_idx);
    start_idx += size_vDEP;
    for (int i = 0; i < size_vDEN; i++)
      dull_normals_[i].first = getPoint(Mobj, i + start_idx);
    start_idx += size_vDEN;
    for (int i = 0; i < size_vDEN; i++)
      dull_normals_[i].second = getPoint(Mobj, i + start_idx);
  }
  else // if it fails to load xml file, calculate and make xml file
  {
    ...
    for (int i = 0; i < sharp_edges_.size(); i++)
      setPoint(&A, i + start_idx, sharp_edges_[i].first);
    start_idx += sharp_edges_.size();
    for (int i = 0; i < sharp_edges_.size(); i++)
      setPoint(&A, i + start_idx, sharp_edges_[i].second);
    start_idx += sharp_edges_.size();
    for (int i = 0; i < dull_edges_.size(); i++)
      setPoint(&A, i + start_idx, dull_edges_[i].first);
    start_idx += dull_edges_.size();
    for (int i = 0; i < dull_edges_.size(); i++)
      setPoint(&A, i + start_idx, dull_edges_[i].second);
    start_idx += dull_edges_.size();
    for (int i = 0; i < dull_normals_.size(); i++)
      setPoint(&A, i + start_idx, dull_normals_[i].first);
