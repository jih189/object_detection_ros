...
public:
  ...
  void refindMatching(cv::Mat &dt, epnp& ePnP);
  float getCMCost(cv::Mat &dt);
  IplImage *extractEdge(IplImage *img, int smoothSize = 1, int cannyLow = 20, int cannyHigh = 40, IplImage *edge = NULL, IplImage *filterImage = NULL);
  ...
  void getVisibleArea(int height, int width, bool onlyEdge, IplImage *output);
...
protected:
  ...
  vector<cv::Point> visible_contours_points_;
