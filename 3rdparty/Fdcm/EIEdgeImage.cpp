...
void EIEdgeImage::Read(LFLineFitter &lf, double rot, double shiftx, double shifty, double scale, double centerx, double centery)
{
	SafeRelease();
	nLines_ = lf.rNLineSegments();
	...
	lines_ = new LFLineSegment[nLines_];
	// no need for support and normal maybe
	for (int i = 0; i < nLines_; i++)
	{
		double trans[2];
		lines_[i] = lineSegmentMap[i];
