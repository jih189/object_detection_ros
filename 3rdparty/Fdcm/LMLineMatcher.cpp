...
LMLineMatcher::~LMLineMatcher() { SafeRelease(); };

void LMLineMatcher::SafeRelease() {
  if (dbImages_) delete[] dbImages_;
  dbImages_ = NULL;
};
...
void LMLineMatcher::Match(LFLineFitter &lf, vector<LMDetWind> &detWind) {
  ...
  // std::cout<<"IDT3 Computation Time
  // "<<setiosflags(ios::fixed)<<setprecision(6)<<(t2.QuadPart -
  // t1.QuadPart)/(1.0*f.QuadPart)<<std::endl;
  ...
  for (int i = 0; i < ndbImages_; i++) {
    MatchBruteForce(dbImages_[i], i, iindices, indices, xIndices, yIndices,
                    dIndices, sIndices, distances, counter, minCost);
  }

  ////QueryPerformanceCounter(&t2);
  // std::cout<<"Fast Directional Chamfer Matching Time
  // "<<setiosflags(ios::fixed)<<setprecision(6)<<(t2.QuadPart -
  // t1.QuadPart)/(1.0*f.QuadPart)<<std::endl;
  ...
  wind.height_ =
      (int)(dbImages_[indices[iindices[0]]].height_ * scale / scale_);
...
double LMLineMatcher::MatchBruteForce(EIEdgeImage &dbImage, int index,
                                      int *iindices, int *indices, int *xIndex,
                                      int *yIndex, double *dIndex,
                                      double *sIndex, double *distances,
                                      int &counter, double &minCost) {
  ...
  for (int s = minSearchScale_; s <= maxSearchScale_; s++) {
    ...
    for (int y = -(int)miny; y < queryImage_.height_ - (int)maxy;
         y += searchStepSize_) {
      for (int x = -(int)minx; x < queryImage_.width_ - (int)maxx;
           x += searchStepSize_) {
        ltrans[0] = (double)x;
        ltrans[1] = (double)y;
        cost = 0;
        ...
        if (minx + ltrans[0] <= searchBoundarySize_ ||
          ...
          for (k = 0; k < tdbImage.nLines_; k++) {
            ...
            sum = queryDistanceImage_.idtImages_[tdbImage.directionIndices_[k]]
                      .Sum((int)line.sx_, (int)line.sy_, (int)line.ex_,
                           (int)line.ey_, currentcount);
            cost += sum * factor;
            // cost+=sum*factor;
...
double LMLineMatcher::MatchBruteForceCostMap(EIEdgeImage &dbImage,
...
void LMLineMatcher::Configure(const char *filename) {
  ...
  //		string substring =
  //line.substr(found+1,line.length()-found).c_str();
...
void LMLineMatcher::PrintParameter() {
  ...
  cout << "* ==========================================================="
       << endl;
  cout << "* NUMBER_DIRECTION:"
       << "\t" << nDirections_ << endl;
  cout << "* DIRECTIONAL_COST:"
       << "\t" << directionCost_ << endl;
  cout << "* MAXIMUM_EDGE_COST:"
       << "\t" << maxCost_ << endl;
  cout << "* MATCHING_SCALE:"
       << "\t" << scale_ << endl;
  cout << "* TEMPLATE_SCALE:"
       << "\t" << db_scale_ << endl;
  cout << "* BASE_SEARCH_SCALE:"
       << "\t" << baseSearchScale_ << endl;
  cout << "* MIN_SEARCH_SCALE:"
       << "\t" << minSearchScale_ << endl;
  cout << "* MAX_SEARCH_SCALE:"
       << "\t" << maxSearchScale_ << endl;
  cout << "* BASE_SEARCH_ASPECT:"
       << "\t" << baseSearchAspect_ << endl;
  cout << "* MIN_SEARCH_ASPECT:"
       << "\t" << minSearchAspect_ << endl;
  cout << "* MAX_SEARCH_ASPECT:"
       << "\t" << maxSearchAspect_ << endl;
  cout << "* SEARCH_STEP_SIZE:"
       << "\t" << searchStepSize_ << endl;
  cout << "* SEARCH_BOUNDARY_SIZE:"
       << "\t" << searchBoundarySize_ << endl;
  cout << "* MIN_COST_RATIO:"
       << "\t" << minCostRatio_ << endl;
  cout << "* MODEL_SIZE_BIAS:"
       << "\t" << bias_ << endl;
  cout << "* ==========================================================="
       << endl;
  cout << "*/" << endl;
...
void LMLineMatcher::Init(const char* parentdir, const char* fileName){
  std::cout << "Init" << std::endl;
  fstream file;
  file.open(std::string(parentdir) + "/" + std::string(fileName));

  std::cout << "fileName: " << parentdir << "/" << fileName << std::endl;
  ...
  for (int i = 0; i < ndbImages_; i++) {
    ...
    std::cout << "line#" << i << ": " << line << std::endl;
    dbImages_[i].SetNumDirections(nDirections_);
    dbImages_[i].Read((parentdir + line).c_str());
    dbImages_[i].Scale(scale_ * db_scale_);
  }
  file.close();
}

void LMLineMatcher::SingleShapeDetectionWithVaryingTemplateSizeForROC(
    LFLineFitter &lf, double minThreshold, double gap, double maxThreshold,
  ...
  // cout<<"taking "<<(t2.QuadPart - t1.QuadPart)/(1.0*f.QuadPart)<<"
  // seconds"<<endl;

  MatchingCostMap matchingCostMap;
  int nCostMap = (maxSearchScale_ - minSearchScale_ + 1) *
                 (maxSearchAspect_ - minSearchAspect_ + 1);
  ...
  int counter = 0;
  int i = 0;
  DetectBruteForceVaryingTemplateSize(dbImages_[i], maxThreshold,
                                      matchingCostMap);

  ////QueryPerformanceCounter(&t2);
  // cout<<"Detection Computation";
  // cout<<" ( "<<maxSearchScale_-minSearchScale_+1<<" scales )"<<" taking
  // "<<(t2.QuadPart - t1.QuadPart)/(1.0*f.QuadPart)<<" seconds"<<endl;
  ...
  int nInterval = 0;
  for (double t = minThreshold; t <= maxThreshold; t += gap) nInterval++;

  int index = 0;
  int varyingQuerySize = 0;
  detWindArrays.resize(nInterval);
  for (double t = minThreshold; t <= maxThreshold; t += gap) {
    LMNonMaximumSuppression::ComputeDetection(
        matchingCostMap, t, overlapThreshold, detWindArrays[index],
        varyingQuerySize);
    index++;
  }
  ////QueryPerformanceCounter(&t2);
  // cout<<"Non-maximum suppression ";
  // cout<<"taking "<<setiosflags(ios::fixed)<<setprecision(6)<<(t2.QuadPart -
  // t1.QuadPart)/(1.0*f.QuadPart)<<endl;
}

void LMLineMatcher::SingleShapeDetectionWithVaryingQuerySizeForROC(
    LFLineFitter &lf, double minThreshold, double gap, double maxThreshold,
    vector<vector<LMDetWind> > &detWindArrays) {
  // LARGE_INTEGER t1, t2, f;
  ////QueryPerformanceFrequency(&f);
  ...
  for (int s = minSearchScale_; s <= maxSearchScale_; s++) {
    ...
    DetectBruteForce(dbImages_[i], maxThreshold, scale, matchingCostMap,
                     costMapIndex);
  ...
  // cout<<"Structure computation";
  // cout<<" ( "<<maxSearchScale_-minSearchScale_+1<<" scales )"<<" taking
  // "<<structureCompTime<<" seconds"<<endl;

  // cout<<"Detection Computation";
  // cout<<" ( "<<maxSearchScale_-minSearchScale_+1<<" scales )"<<" taking
  // "<<matchingCostCompTime<<" seconds"<<endl;
...
void LMLineMatcher::DetectBruteForce(EIEdgeImage &dbImage,
                                     double detectionThreshold, double scale,
                                     MatchingCostMap &matchingCostMap,
                                     int &costMapIndex) {
  ...
  for (int a = minSearchAspect_; a <= maxSearchAspect_; a++) {
    ...
    if (min(width, height) == 0) continue;
      ...
      for (int x = -(int)minx; x < queryImage_.width_ - (int)maxx;
        ...
        if (minx + ltrans[0] <= searchBoundarySize_ ||
          ...
          for (int k = 0; k < tdbImage.nLines_; k++) {
            ...
            double sum =
                queryDistanceImage_.idtImages_[tdbImage.directionIndices_[k]]
                    .Sum((int)line.sx_, (int)line.sy_, (int)line.ex_,
                         (int)line.ey_, currentcount);
            cost += sum;
...
// mathingCostMap:
void LMLineMatcher::DetectBruteForceVaryingTemplateSize(
    EIEdgeImage &dbImage, double detectionThreshold,
    MatchingCostMap &matchingCostMap) {
  int costMapIndex = 0;
  int count = 0;
  int currentcount = 0;
  ...
  for (int s = minSearchScale_; s <= maxSearchScale_;
       s++)  // detect with different size of the template
  {
    scale = baseSearchScale_ * s;
    for (int a = minSearchAspect_; a <= maxSearchAspect_;
         a++)  // detection with different aspect ratios
    {
      EIEdgeImage tdbImage;
      tdbImage = dbImage;
      ...
      tdbImage.Boundary(minx, miny, maxx,
                        maxy);  // get the new boundary of the template
      tdbImage.Tight(
          minx, miny, maxx,
          maxy);  // set the template to the up left corner of the image
      tdbImage
          .SetDirectionIndices();  // get the direction of each line in template

      factor = 1.0 / pow(tdbImage.Length(),
                         bias_);  // factor = 1 / template total pixel number

      double currentDetectionThreshold =
          queryDistanceImage_.maxCost_ * detectionThreshold / factor;

      width = (queryImage_.width_ - (int)(maxx - minx)) /
              searchStepSize_;  // find width range
      height = (queryImage_.height_ - (int)(maxy - miny)) /
               searchStepSize_;  // find height range
      width = max(0, width);
      height = max(0, height);
      ...
      matchingCostMap.width_[costMapIndex] =
          width;  // set width range and height range to the cost map
      matchingCostMap.height_[costMapIndex] = height;
      ...
      for (int y = -(int)miny; y < queryImage_.height_ - (int)maxy;
        ...
        for (int x = -(int)minx; x < queryImage_.width_ - (int)maxx;
          ...
          matchingCostMap.costMap_[costMapIndex][imageIdx] =
              cost;  // costMapIndex is the combinition of the size and other;
                     // imageIdx is position of the template
          x0++;
        }
        y0++;
      }
...
void LMLineMatcher::SingleShapeDetectionWithVaryingQuerySize(
    LFLineFitter &lf, int tidx, double maxThreshold,
    vector<LMDetWind> &detWindArrays) {
  // LARGE_INTEGER t1, t2, f;
  ////QueryPerformanceFrequency(&f);
  double scale;
  // double structureCompTime = 0;
  ...
  for (int s = minSearchScale_; s <= maxSearchScale_; s++) {
    ...
    DetectBruteForce(dbImages_[tidx], maxThreshold, scale, matchingCostMap,
                     costMapIndex);
  ...
  LMNonMaximumSuppression::ComputeDetection(matchingCostMap, maxThreshold,
                                            overlapThreshold, detWindArrays,
                                            varyingQuerySize);
  index++;
  ////QueryPerformanceCounter(&t2);
  // cout<<"Non-maximum suppression ";
...
void LMLineMatcher::MultiShapeDetectionWithVaryingQuerySize(
    LFLineFitter &lf, double maxThreshold, vector<LMDetWind> &detWindArrays) {
...
#pragma omp parallel for
    for (int t = 0; t < ndbImages_; t++) {
      DetectBruteForce(dbImages_[t], maxThreshold, scale, matchingCostMap[t],
                       costMapIndex[t]);
    }
  }
  cout << "]" << endl;
  ...
  for (int t = 0; t < ndbImages_; t++) {
    LMNonMaximumSuppression::ComputeDetection(matchingCostMap[t], maxThreshold,
                                              overlapThreshold, detWind,
                                              varyingQuerySize);
  ...
  for (size_t i = 0; i < detWindAll.size(); i++) tmpWind[i] = detWindAll[i];
...
void LMLineMatcher::MultiShapeDetectionWithVaryingTemplateSize(
    LFLineFitter &lf, double maxThreshold, vector<LMDetWind> &detWindArrays) {
  double scale;
  vector<MatchingCostMap> matchingCostMap;
  matchingCostMap.resize(ndbImages_);
...
#pragma omp parallel for num_threads(10)
  for (int t = 0; t < ndbImages_; t++) {
    // std::cout << ".";
    DetectBruteForceVaryingTemplateSize(dbImages_[t], maxThreshold,
                                        matchingCostMap[t]);
  }
